...
const CardUpcomingCinema = ({ data, currentUser, position }: { data: any, currentUser?: User | null, position: "right" | "left" }) => {
    const formattedDate = (dateTime: Date) => {
        return format(new Date(dateTime), "dd MMM yyyy");
    };
    const { hasFavorited, toggleFavorite } = useFavorites({ currentUser: currentUser, movieId: data.id, });
    return (
        <div className={`w-fit relative flex gap-6 md:gap-10 lg:gap-20 overflow-hidden `}>
            {/* Image */}
            {position === "left" &&
                <>
                    <Image
                    ...
                    <button className="absolute top-4 lg:top-5 left-24 md:left-40 lg:left-56" onClick={toggleFavorite}>
                        <HeartIcon
                            style={`w-5 h-5 lg:w-6 lg:h-6 ${hasFavorited ? "fill-rose-500 text-rose-500" : "fill-neutral-500/70"} hover:fill-red-heart transition duration-300`} />
                    </button>
                </>}
            <div className="flex w-full flex-col justify-between py-2 lg:py-5">
                ...
                <Link href={`/movies/${data.id}`} className="font-semibold text-red text-sm lg:text-xl">Read More ...</Link>
            </div>
            {position === "right" &&
                <>
                    <Image
                        ...
                        className={`object-center object-cover  w-[130px] md:w-[230px] md:h-[300px] lg:w-[274px] h-[220px] lg:h-[408px] transition duration-300 rounded-2xl`}
                    />
                    <button className="absolute top-4 right-3 lg:right-6" onClick={toggleFavorite}>
                        <HeartIcon
                            style={`w-5 h-5 lg:w-6 lg:h-6 ${hasFavorited ? "fill-rose-500 text-rose-500" : "fill-neutral-500/70"} hover:fill-red-heart transition duration-300`} />
                    </button>
                </>}
